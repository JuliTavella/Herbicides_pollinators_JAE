...
####CONTROL NO REWIRING - la spp no puede cambiar de planta hospedera
...
new_reconectar_without_rewiring2<-function(nueva_comunidad, comunidad_anterior, red_anterior){
	...
	#estimates new pollinators frequency 
		...
		red_new=empty(nueva_red(nueva_comunidad, nuevas_abundancias, red_anterior))
			...
			if(is.array(red_new)==TRUE){red_final=c(dim(red_new),"small")}else{red_final=c(length(red_new),1,"small")}
			#red_final="small"
			}
...
#save table
write.table(tabla_final_modifiers, file = "G:\\Mi unidad\\Herbicides_pollination_Julia\\JAE\\JAE R1\\CONTROL_NO_REWIRING_SC1.csv", sep = ",", row.names=F)
